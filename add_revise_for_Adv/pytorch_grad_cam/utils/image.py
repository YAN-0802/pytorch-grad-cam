...
def make_uniform_heatmap(heatmap, square=7):
    # 确保输入的heatmap是PyTorch tensor
    heatmap_tensor = torch.tensor(heatmap, dtype=torch.float32)
    # heatmap = torch.as_tensor(heatmap)

    # 获取热图的大小
    height, width = heatmap_tensor.shape[0], heatmap_tensor.shape[1]
    # 初始化一个新的相同大小的tensor用于存储结果
    if height % square != 0 or width % square != 0: #"注意力热图的高度和宽度必须是7的倍数"
        print("Wrong division!")
    else:
        window_size = height//square
        # 遍历每个块
        for i in range(0, height, window_size):
            for j in range(0, width, window_size):
                window = heatmap_tensor[i:i + window_size, j:j + window_size]
                mean_value = window.mean(dim=(0, 1), keepdim=True)
                heatmap_tensor[i:i + window_size, j:j + window_size] = mean_value
        uniform_heatmap = heatmap_tensor.numpy()

        return uniform_heatmap
